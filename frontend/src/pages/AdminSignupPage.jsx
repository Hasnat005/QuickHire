...
function AdminSignupPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const authData = await signupAdmin({
        name: name.trim(),
        email: email.trim(),
        password,
      })
      ...
      setError('Unable to create admin account. Try a different email.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="mx-auto max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
      <h1 className="text-2xl font-bold tracking-tight text-[#243251]">Admin Signup</h1>
      <p className="mt-2 text-sm text-slate-500">Create an admin account to manage jobs.</p>
      ...
      <p className="mt-4 text-sm text-slate-600">
        ...
        <Link to="/admin/login" className="font-semibold text-indigo-600 hover:text-indigo-700">
          Login
        </Link>
      </p>
    </div>
